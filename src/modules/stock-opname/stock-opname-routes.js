...
router.get("/no-stock-opname/:noso", verifyToken, async (req, res) => {
  ...
  console.log(
    `[${new Date().toISOString()}] Stock opname - ${username} mengakses : "${filterBy}"`,
  );
  ...
  try {
    ...
    // Ambil data lokasi
    const mstLokasiResult = await request.query(
      "SELECT IdLokasi, Blok FROM MstLokasi WHERE Enable = 1",
    );

    if (!mstLokasiResult.recordset || mstLokasiResult.recordset.length === 0) {
      return res
        .status(404)
        .json({ message: "Data MstLokasi tidak ditemukan" });
    }
    ...
    if (filterBy && filterBy !== "all") {
      const filterMap = {
        st: {
          table: "StockOpname_Hasil_d_ST",
          column: "NoST",
          type: "Sawn Timber",
        },
        sanding: {
          table: "StockOpname_Hasil_d_Sanding",
          column: "NoSanding",
          type: "Sanding",
        },
        s4s: { table: "StockOpname_Hasil_d_S4S", column: "NoS4S", type: "S4S" },
        moulding: {
          table: "StockOpname_Hasil_d_Moulding",
          column: "NoMoulding",
          type: "Moulding",
        },
        laminating: {
          table: "StockOpname_Hasil_d_Laminating",
          column: "NoLaminating",
          type: "Laminating",
        },
        fj: {
          table: "StockOpname_Hasil_d_FJ",
          column: "NoFJ",
          type: "Finger Joint",
        },
        ccakhir: {
          table: "StockOpname_Hasil_d_CCAkhir",
          column: "NoCCAkhir",
          type: "CC Akhir",
        },
        bj: {
          table: "StockOpname_Hasil_d_BJ",
          column: "NoBJ",
          type: "Barang Jadi",
        },
      };
      ...
      query = `
            ...
            `;
      totalCountQuery = createTotalQuery(
...
router.post("/no-stock-opname/:noso/scan", verifyToken, async (req, res) => {
  ...
  if (!noso || !resultscanned || !idlokasi) {
    return res
      .status(400)
      .json({ message: "NoSO, resultscanned, and idlokasi are required." });
  }
  ...
  try {
    ...
    const tableMap = {
      E: {
        tableName: "StockOpname_Hasil_d_ST",
        columnName: "NoST",
        tableH: "ST_h",
        isColumn: "IsST",
      },
      R: {
        tableName: "StockOpname_Hasil_d_S4S",
        columnName: "NoS4S",
        tableH: "S4S_h",
        isColumn: "IsS4S",
      },
      S: {
        tableName: "StockOpname_Hasil_d_FJ",
        columnName: "NoFJ",
        tableH: "FJ_h",
        isColumn: "IsFJ",
      },
      T: {
        tableName: "StockOpname_Hasil_d_Moulding",
        columnName: "NoMoulding",
        tableH: "Moulding_h",
        isColumn: "IsMoulding",
      },
      U: {
        tableName: "StockOpname_Hasil_d_Laminating",
        columnName: "NoLaminating",
        tableH: "Laminating_h",
        isColumn: "IsLaminating",
      },
      V: {
        tableName: "StockOpname_Hasil_d_CCAkhir",
        columnName: "NoCCAkhir",
        tableH: "CCAkhir_h",
        isColumn: "IsCCAkhir",
      },
      W: {
        tableName: "StockOpname_Hasil_d_Sanding",
        columnName: "NoSanding",
        tableH: "Sanding_h",
        isColumn: "IsSanding",
      },
      I: {
        tableName: "StockOpname_Hasil_d_BJ",
        columnName: "NoBJ",
        tableH: "BarangJadi_h",
        isColumn: "IsBJ",
      },
    ...
    if (headerResult.recordset.length === 0) {
      return res
        .status(404)
        .json({ message: "Column not found in StockOpname_h." });
    }
    ...
    if (!isColumnEnabled) {
      return res
        .status(403)
        .json({ message: `${columnName} Tidak di Aktifkan!` });
    }
    ...
    if (count > 0) {
      ...
      return res.status(422).json({
        message: `Data Duplikat! Label Telah Terdaftar di ${existingIdLokasi}`,
      });
    } else {
    ...
    res
      .status(500)
      .json({ message: "Failed to insert/update data.", error: error.message });
  }
});
